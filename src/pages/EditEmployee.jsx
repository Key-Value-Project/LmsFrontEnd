...
const EditEmployee = () => {
  ...
  const [
    updateEmployee,
    { isSuccess: updateSuccess, isError, data: updateData, error },
  ] = useUpdateEmployeeMutation();
  const [
    updateEmployeeRelation,
    {
      isSuccess: updateSuccessRelation,
      isErrorRelation,
      data: updateDataRelation,
      errorRelation,
    },
  ] = useUpdateEmployeeRelationMutation();
    ...
    if (updateSuccess || updateSuccessRelation) {
      ...
      if (isErrorRelation) {
        let notification =
          ...
          (errorRelation.data.errors.length > 0
            ? ": " + errorRelation.data.errors.join(", ")
            : "");
        notifyError(notification);
      }
    }
